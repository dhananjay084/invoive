...
const ModalComponent = ({ isOpen, onClose, onSubmit }) => {
  ...
  const isHMSelected = (formData.invoicePrefix || "").startsWith("HM_");
...
<div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
  <select 
    ...
    onChange={(e) => {
      ...
      const suffix = stripInvoicePrefix(formData.invoiceNo);
      setFormData((prev) => {
        const wasHM = (prev.invoicePrefix || "").startsWith("HM_");
        const willHM = prefix.startsWith("HM_");
        const next = { 
          ...prev, 
  ...
  <input
    ...
    value={stripInvoicePrefix(formData.invoiceNo)}
    onChange={(e) => {
...
<input
  ...
  onChange={(e) => {
    ...
    if (selectedCustomer) {
      setFormData((prev) => {
        const hm = (prev.invoicePrefix || "").startsWith("HM_");
        return ({
          ...prev,
          address1: selectedCustomer.address1,
          address2: selectedCustomer.address2,
          phone: selectedCustomer.pincode,
          clientGST: hm ? "" : selectedCustomer.gst,
          panNo: hm ? "" : selectedCustomer.panNo
        });
      });
    }
  }}
...
<datalist id="customerOptions">
 ...
 <label>Address Line 3:</label>
        ...
        <label>{isHMSelected ? "VAT No:" : "GST No:"}</label>
        <input 
          type="text" 
          name="clientGST" 
    ...
    <>
      ...
      <table>
        ...
        {isHMSelected ? (
          <div>
            <label>VAT (%):</label>
            <input type="number" name="vatTax" value={formData.vatTax} onChange={handleChange} />
          </div>
        ) : (
          <>
            <div>
